...
class EntryResponse(Uuid, Timestamp, BaseModel):
...
class PrivateEntryPreviewResponse(EntryResponse, DebugModelName, BaseModel):
    is_public: bool
...
class PublicEntryDetailsResponse(EntryResponse, DebugModelName, BaseModel):
    views: list[PublicViewResponse]

    model_config = ConfigDict(from_attributes=True)


class PrivateEntryDetailsResponse(EntryResponse, DebugModelName, BaseModel):
    is_public: bool
