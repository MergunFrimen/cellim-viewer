import { 
  ...
  export const entriesApi = {
    ...
    list: async ({ search, page = 1, per_page = 10 }: SearchParams): Promise<EntryListResponse> => {
    ...
    listPublic: async ({ search, page = 1, per_page = 10 }: SearchParams): Promise<EntryListResponse> => {
      const params = new URLSearchParams();
      if (search) params.append('search', search);
      params.append('page', page.toString());
      params.append('per_page', per_page.toString());
      
      const response = await fetch(`${API_BASE_URL}/entries/public?${params.toString()}`);
      return handleResponse<EntryListResponse>(response);
    },
    ...
    getBySharing: async (uuid: string): Promise<Entry> => {
      const response = await fetch(`${API_BASE_URL}/entries/by-sharing-uuid/${uuid}`);
      return handleResponse<Entry>(response);
    },
    
    // Get entry by edit UUID
    getByEdit: async (uuid: string): Promise<Entry> => {
      const response = await fetch(`${API_BASE_URL}/entries/by-edit-uuid/${uuid}`);
      return handleResponse<Entry>(response);
    },
